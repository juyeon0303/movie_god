...
const MOOD_PRESETS = [
  ...
  { label: "명작", query: "비평가 점수가 높은 명작" },
  { label: "비주얼", query: "영상미가 좋은 명작" },
  { label: "비 오는 날", query: "비 오는 날 혼자 보기 좋은 영화" },
  { label: "스트레스", query: "스트레스 풀기 좋은 영화" },
];
    ...
    try {
      ...
      if (!res.ok) {
        if (data.syncRequired) {
          setError("이 OTT 데이터가 아직 없어요. 잠시 후 다시 시도해 주세요.");
        } else {
      ...
      setError("연결에 실패했어요. 서버가 시작 중일 수 있어요.");
      setResults([]);
      setInterpretation("");
      setMethod(null);
    } finally {
    ...
    >
      <div className="border-b border-panel-border px-4 py-4 sm:px-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-ui text-sm font-semibold text-gold">무드 검색</h2>
        </div>
        <p className="font-ui mt-1 text-sm text-panel-muted">
          기분이나 상황을 입력하면 Approved 목록에서 맞는 작품을 찾아요
        </p>
      </div>
